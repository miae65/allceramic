...
export function JobCommentSection({ postId, initialComments, currentUserId, isAdminUser }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setComments(prev => [...prev, data as JobComment])
      setNewComment('')
      router.refresh()
    } catch (err) {
  ...
  const onDelete = async (commentId: string) => {
    ...
    try {
      const supabase = createClient()
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: e } = await (supabase as any).from('job_comments').delete().eq('id', commentId)
      if (e) throw new Error(e.message)
