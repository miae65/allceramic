...
import type { ExhibitionComment } from '@/types'
...
export function ExhibitionCommentSection({ postId, initialComments, currentUserId, isAdminUser }: Props) {
  const router = useRouter()
  const { user } = useAuth()
  const [comments, setComments] = useState<ExhibitionComment[]>(initialComments)
  ...
  const submit = async () => {
    ...
    try {
      ...
      setComments(prev => [...prev, data as ExhibitionComment])
      setNewComment('')
      router.refresh()
    } catch (err) {
  ...
  const onDelete = async (commentId: string) => {
    ...
    try {
      const supabase = createClient()
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: e } = await (supabase as any).from('exhibition_comments').delete().eq('id', commentId)
      if (e) throw new Error(e.message)
