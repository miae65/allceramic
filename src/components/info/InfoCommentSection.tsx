...
import type { InfoComment } from '@/types'

type Props = {
  postId: string
  initialComments: InfoComment[]
...
export function InfoCommentSection({ postId, initialComments, currentUserId, isAdminUser }: Props) {
  const router = useRouter()
  const { user } = useAuth()
  const [comments, setComments] = useState<InfoComment[]>(initialComments)
  ...
  const submit = async () => {
    ...
    try {
      ...
      setComments(prev => [...prev, data as InfoComment])
      setNewComment('')
      router.refresh()
    } catch (err) {
  ...
  const onDelete = async (commentId: string) => {
    ...
    try {
      const supabase = createClient()
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: e } = await (supabase as any).from('info_comments').delete().eq('id', commentId)
      if (e) throw new Error(e.message)
